...
class CNN:
    ...
    def __init__(self, training_images, training_labels, layers, classes):
    ...
    def feedforward(self, input, class_index):
        output = self.feedforward_output(input)
    ...
    def train_cycle(self, input, label, rate):
        class_index = self.classes.index(label)

        output, loss, correct = self.feedforward(input, class_index)

        loss_gradient = np.zeros(len(self.classes))
        loss_gradient[class_index] = -1 / output[class_index]

        self.backpropagate(loss_gradient, rate)

        return loss, correct

    def train(self, num_epochs=10, rate=0.001):
        ...
        for epoch in range(num_epochs):
            ...
            for i, (input, label) in enumerate(zip(t_inputs, t_labels)):
                i_loss, i_correct = self.train_cycle(input, label, rate)
